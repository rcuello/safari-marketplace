...
- **Size budget**: Proposal artifact MUST be under 1450 words. This is a deliberate conciseness budget set by the repo owner, NOT a ceiling derived from precedent: the 18 archived `proposal.md` in `openspec/changes/archive/` range from 839 to 3702 words (median ≈2200; measured 2026-09-14 with `wc -w openspec/changes/archive/*/proposal.md | sort -n`), so most of them EXCEED the budget — do not use them as a size reference. The two shortest (839 and 1431 words) prove the budget is achievable while still carrying the rollback plan, scope table, and capability contract this repo requires. The budget guards against padding, not against evidence: use bullet points and tables over prose. Headers organize, not explain.
