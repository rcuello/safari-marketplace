# Product Detail API Specification

## Purpose

`GET /api/products/:slug` lee `products` en Postgres vía `@safari/db` en vez de
`products.json`, devolviendo detalle de un producto incluyendo sus productos relacionados.
...
(`packages/db/src/repositories/products.repository.ts:227-253`), no de `products.json`.
`ProductsService.getProductBySlug` (`apps/api/rest/src/products/products.service.ts:210`)
MUST ser `async`. El objeto raíz MUST tener las 20 claves de `product-listing-api`
...
`visibility` (`packages/db/src/repositories/products.repository.ts:237-247`
previo); esta regla los elimina para replicar byte a byte el mock
(`apps/api/rest/src/products/products.service.ts:210-219` previo). En esta
US **CA-1 (paridad de contrato) gana sobre la corrección de UX** — decisión
ratificada, no reabrir.
...
(`apps/api/rest/src/products/products.service.ts:200-207`); otro error no
controlado MUST responder 500 con el mismo helper. El `try/catch` MUST NOT
capturar ni re-envolver el `NotFoundException` anterior.
