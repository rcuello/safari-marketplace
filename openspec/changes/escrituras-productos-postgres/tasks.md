...
- [x] 4.2 Cubrir `create`/`update`/`remove`: proyección de 20 claves en el mismo orden que `getProduct`; `Number()` de las 3 FK desde body string; id `NaN`/no-entero-seguro/`<=0` → 404 sin llamar al repositorio (nivel A); `parent`/`shop_id` `null` explícito manejado sin `Number(null)===0` (paralelo a DD28-10). **Hallazgo real (ver apply-progress.md § GATE — defecto genuino en `products.service.ts`, fuera de alcance de este archivo):** `manufacturer_id: null` explícito SÍ debería llegar como `null` al repositorio (R29-7); el servicio migrado en PR#3 lo convierte en `Number(null) === 0` en `create()` (y el mismo patrón existe en `update()`). El test escrito para probar el contrato correcto queda en **rojo**, a propósito — no se debilitó ni se borró.
...
- [ ] 4.5 Verificar PR#4: `cd apps/api/rest && npx jest` verde. **NO verde** — 203/204 en verde, 1 test en rojo por un defecto genuino de `products.service.ts` (fuera del alcance de este archivo; ver apply-progress.md). Conteo real pegado (204 tests, 9 suites — no el "4 suites / 65 tests" obsoleto de `CLAUDE.md`, sin corregirlo). El archivo NO desborda el forecast (~650): +387 líneas reales. Bloqueado en espera de decisión del orquestador/mantenedor sobre el defecto (no decidido unilateralmente).
